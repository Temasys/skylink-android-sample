...
public class DataChannelManager {
    ...
    // Time out (seconds) for file transfers.
...
// -------------------------------------------------------------------------------------------------
    ...
    public DataChannel createDataChannel(
        ...
        // Add tid and dcObserver pair to dcInfoList
    ...
    public void disposeDC(String peerId) {
        ...
        while (iPeerId.hasNext()) {
            ...
            // Exist loop if required DC has been closed and disposed.
            if (peerId != null) {
...
// -------------------------------------------------------------------------------------------------
    ...
    */
        ...
        // Trigger callback
        if (!isPeerIdMCU(dcObserver.getTid()))
            ((Activity) connectionManager.getContext()).runOnUiThread(new Runnable() {
                public void run() {
    ...
    public void dataChannelDATAHandler(final DcObserver dcObserver, String dataStr) {
        ...
        double pctTemp = (double) chunk * (double) CHUNK_SIZE_POST / (double) fileSize * 100;
        ...
        if (pctTemp > 100) pctTemp =
                (double) (chunk - 1) * (double) CHUNK_SIZE_POST / (double) fileSize * 100;
    ...
    */
        ...
        if (pos == fileSize) {
            ...
            sendDcString(dcObserver.getDc(), "");
            sendLast = true;
        } else if (pos > fileSize) {
            // This is the receiver acknowledging that he got the last chunk.
        ...
        if (!sendLast) {
            ...
            sendDcString(dcObserver.getDc(), chunkStr);
        }
...
// -------------------------------------------------------------------------------------------------
    ...
    private void sendWRQ(DcObserver dcObserver, String uA, String fileName,
    ...
    isPrivate = Number of seconds for our response to be received before timeout.
    */
        // String fileSizeEncoded = Long.toString( dcObserver.getFileSizeEncoded() );
        // String chunkSize = Integer.toString( CHUNK_SIZE_POST );
    ...
    private void sendACK(DcObserver dcObserver, long chunk) {
    ...
    */
        ...
        String msgStr = msgJson.toString();
        sendDcString(dcObserver.getDc(), msgStr);
    }
...
// -------------------------------------------------------------------------------------------------
    ...
    private String getDisplayName(String tid) {
        String nick = "";
        try {
            JSONObject json = (JSONObject) connectionManager.getUserData(tid);
            if (json != null) {
                nick = (json).getString("displayName");
            }
        } catch (JSONException e) {
            Log.e(TAG, e.getMessage(), e);
        }
